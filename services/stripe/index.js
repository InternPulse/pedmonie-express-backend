...
const { createUnpaidOrder, updateOrderStatus, createTransaction, creditWallet } = require('../../controller/stripe/index')
...
const orderCheckout = async (req, res) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      // metadata: {
      //   order_id: order.order_id,// giving me issues: order is undefined
      //   merchant_id: merchantId,
      // },
    })
      //console.log(order)
  ...
  const { session_id } = req.params
    try {
      ...
      if (session.payment_status === "paid") {
        const updatedOrder = await updateOrderStatus(order_id, "successful", {
          transaction: t,
        })
